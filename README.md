...
- **Export** (`/export`, linked from a card on Home rather than the bottom
  tab bar -- see "Data export" below) -- download the full transaction
  ledger as CSV, or every table as one JSON file.

Home also has a **"Statements needed" checklist** (see "The statement
coverage checklist" below), between the Status card and the Run Pipeline
Now button: one row per known account showing whether it has a recent
statement on file.
...
### The statement coverage checklist

Now that the household's records live in Supabase instead of a workbook
they update by hand, nothing naturally reminds them "you haven't uploaded
August's Amex statement yet" the way noticing a blank row used to.
`dashboard/coverage.py: get_account_coverage()` closes that gap: for each
of the 13 known accounts (`pipeline/accounts.py`'s `ALL` list), it finds
that account's most recent successfully-processed `statement_period` and
flags it overdue if that's more than `STALE_THRESHOLD_DAYS` (45 -- roughly
one monthly cycle plus a buffer for generation/upload lag) days old, or if
it has never had one. This decision is **entirely deterministic** -- plain
date arithmetic against `processed_statements`, no LLM involved -- because
getting "is this overdue" wrong is worse than an unpolished sentence
either way (a false negative hides a real gap; a false positive nags about
an account that's actually current).

A short, friendly one-line summary at the top of the checklist (e.g. "2
accounts are overdue: Amex, Costco.") is generated by
`llm_headline()` -- the same compute-in-Python-then-let-the-LLM-phrase-it
split used by the Ask page, model `claude-haiku-4-5`. The model is given
the already-computed coverage list and asked only to phrase it, never to
decide what's overdue; a missing `ANTHROPIC_API_KEY`, a slow/failed call
(8s timeout), or an empty response all fall back to a plain Python-built
sentence (`_fallback_headline()`), so the checklist itself -- the actual
per-account data -- never depends on the LLM being configured or
reachable, only the wording of its summary line does.

Matching is by substring against `processed_statements.account_name`
because a statement covering more than one account (e.g. a combined
Capital One 360 Checking+Savings PDF) is stored there as both names joined
with `" & "` (see `pipeline/main.py: account_name_for_statement`), not as
a list; none of the 13 real account names are substrings of each other,
so this is unambiguous. There's no "account closed" flag in the schema, so
all 13 accounts in `pipeline/accounts.py` are treated as active -- if one
is ever formally retired, remove it from that list (it's already the
source of truth other pipeline code depends on) so it stops appearing here.

### Data export

The household's previous source of truth was one Excel workbook they
could open, copy, or hand to any other tool -- including pasting into an
LLM for ad hoc analysis. Moving to Supabase must not lose that
capability, so `/export` (linked from a card on Home) offers two
downloads, both built from the same paginated `_fetch_all` queries used
everywhere else in `dashboard/db.py` (never an unpaginated `.execute()`,
which would silently truncate at 1000 rows):

- **`transactions.csv`** (`/export/transactions.csv`) -- every
  transaction, one row per line, in the same column shape as the old
  workbook's main sheet (date, merchant, amount, category, ...). CSV is
  the most broadly compatible format both for spreadsheet tools and for
  uploading directly into an LLM.
- **`full.json`** (`/export/full.json`) -- every table this app reads
  from (`accounts`, `category_keys`, `processed_statements`,
  `transactions`, `needs_review`, `assets_liabilities`,
  `income_paychecks`), each as a list of rows, in one JSON file -- for a
  complete backup or for handing an LLM the whole dataset (not just the
  ledger) at once.

Both are built in `dashboard/export.py` and are just password-gated
downloads (`Content-Disposition: attachment`) -- no new tables, no
scheduling, nothing written back to Supabase.
...
- **Statement coverage threshold (`STALE_THRESHOLD_DAYS = 45`)**: chosen by
  looking at real `processed_statements` history -- every account cycles
  roughly monthly (closing-day-of-month varies by account, e.g. Citi
  Costco closes near the 1st-3rd rather than month-end, but the ~30-day
  cadence doesn't), so 45 days gives one full cycle plus a couple weeks'
  buffer before nagging. One threshold for all 13 accounts, since the
  schema has no per-account cycle-length field. See "The statement
  coverage checklist" above.
